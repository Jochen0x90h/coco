...
/// G0X0 https://www.st.com/en/microcontrollers-microprocessors/stm32g0-series/documentation.html Section 28
/// G0X1 https://www.st.com/en/microcontrollers-microprocessors/stm32g0-series/documentation.html Section 37
/// H503 https://www.st.com/en/microcontrollers-microprocessors/stm32h5-series/documentation.html Section 40
/// H5 https://www.st.com/en/microcontrollers-microprocessors/stm32h5-series/documentation.html Section 55
/// U3 https://www.st.com/en/microcontrollers-microprocessors/stm32u3-series/documentation.html Section 52
namespace usbd {

// unified defines for EP register

// endpoint address
#define USB_EP_ADDR USB_CHEP_ADDR

// endpoint type
#define USB_EP_TYPE USB_EP_UTYPE

// RX status
#define USB_EP_STAT_RX USB_EP_RX_STRX

// RX correct transfer
#define USB_EP_CTR_RX USB_EP_VTRX

// TX status
#define USB_EP_STAT_TX USB_EP_TX_STTX
...
struct Instance {
    ...
    USB_DRD_TypeDef *operator ->() const {return usb;}
    operator USB_DRD_TypeDef *() const {return usb;}
    ...
    auto &configure(Array<const BufferCapacity> bufferCapacities) {
        ...
        for (auto &capacity : bufferCapacities) {
            ...
            desc->rx = offset | (int(capacity.rx) << 16);
    ...
    auto &reset() {
        ...
        usb->CHEP0R = USB_EP_RX_VALID | USB_EP_TX_STALL | USB_EP_CONTROL | 0;
    ...
    int applyAddress() {
        auto desc = reinterpret_cast<volatile EndpointDescriptor *>(ram);
        int offset = desc[0].rx & 0xffff;
        auto data = (volatile uint32_t *)(intptr_t(ram) + offset);
        int address = data[0] >> 16;
        usb->DADDR = USB_DADDR_EF | address;
        return address;
    }

    auto &endpointRegister(int ep) {
        return (&usb->CHEP0R)[ep];
    }
    ...
    void controlStatusIn() {
        auto desc = reinterpret_cast<volatile EndpointDescriptor *>(ram);
        desc[0].tx = desc[0].tx & 0xffff; // clear tx size of control endpoint

        // keep DTOG_RX DTOG_TX STAT_RX, cancel STAT_TX, clear CTR_RX CTR_TX, toggle STAT_TX.VALID
        usb->CHEP0R = ((usb->CHEP0R & ~(USB_EP_DTOG_RX | USB_EP_DTOG_TX | USB_EP_STAT_RX | USB_EP_CTR_RX | USB_EP_CTR_TX)) ) ^ USB_EP_TX_VALID;
    }
    ...
    void controlStatusOut() {
        ...
        usb->CHEP0R = ((usb->CHEP0R & ~(USB_EP_DTOG_RX | USB_EP_DTOG_TX | USB_EP_STAT_TX | USB_EP_CTR_RX | USB_EP_CTR_TX)) | USB_EP_KIND) ^ USB_EP_RX_VALID;
    }
    ...
    void controlStall() {
        ...
        usb->CHEP0R = ((usb->CHEP0R & ~(USB_EP_DTOG_RX | USB_EP_DTOG_TX | USB_EP_CTR_RX | USB_EP_CTR_TX | USB_EP_KIND)) ) ^ (USB_EP_RX_STALL | USB_EP_TX_STALL);
    }
    ...
    int rx(int ep, uint32_t *data, int size) {
        auto desc = reinterpret_cast<volatile EndpointDescriptor *>(ram);
        uint32_t os = desc[ep].rx;

        // get offset
        int offset = os & 0xffff;

        // get number of received bytes
        size = std::min(int((os >> 16) & 0x3ff), size);

        // copy data from rx buffer into memory
        auto src = (const volatile uint32_t *)(uintptr_t(ram) + offset);
        auto dst = data;
        auto end = dst + ((size + 3) >> 2);
        while (dst != end) {
            *dst = *src;
            ++src;
            ++dst;
    ...
    void tx(int ep, const uint32_t *data, int size) {
        auto desc = reinterpret_cast<volatile EndpointDescriptor *>(ram);
        volatile uint32_t &os = desc[ep].tx;
        ...
        auto src = data;
        auto dst = (volatile uint32_t *)(uintptr_t(ram) + offset);
        auto end = dst + ((size + 3) >> 2);
        while (dst != end) {
            *dst = *src;
            ++src;
    ...
    std::pair<T, uint8_t> getSetupRequest() {
        auto desc = reinterpret_cast<volatile EndpointDescriptor *>(ram);
        int offset = desc[0].rx & 0xffff;
        auto data = (volatile uint32_t *)(intptr_t(this->ram) + offset);
        int d = data[0];
    ...
    T getSetup() {
        auto desc = reinterpret_cast<volatile EndpointDescriptor *>(ram);
        int offset = desc[0].rx & 0xffff;
        auto data = (volatile uint32_t *)(uintptr_t(this->ram) + offset);
        auto d0 = data[0];
        auto d1 = data[1];
        return {R(d0), uint8_t(d0 >> 8), uint16_t(d0 >> 16), uint16_t(d1), uint16_t(d1 >> 16)};
    }
};
