...
/// Registers for reset, clock in run mode, clock in sleep/stop mode.
namespace rcc {
...
struct Info {
    ...
    /// @brief Enable clock of a core in sleep and stop mode.
    ///
    void enableClockSleepStop() const {
        auto reg = (__IO uint32_t *)(intptr_t(this->enableReg) + clockSleepStopOffset);
        *reg  = *reg | this->flag;
    }

    /// @brief Disable clock of a core in sleep and stop mode.
    ///
    ...
    static constexpr int clockSleepStopOffset = (offsetof(RCC_TypeDef, AHB1SMENR) - offsetof(RCC_TypeDef, AHB1ENR));
};
...
struct Info2 {
    ...
    ///
    void enableClockSleepStop() const {
        auto reg = (__IO uint32_t *)(RCC_BASE + R + clockSleepStopOffset);
        *reg = *reg | this->flag;
    }
