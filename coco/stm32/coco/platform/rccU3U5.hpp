...
/// Registers for reset, clock in run mode, clock in sleepmode, clock in stop mode.
///
/// U3 https://www.st.com/en/microcontrollers-microprocessors/stm32u3-series/documentation.html Section 10
namespace rcc {
...
struct Info {
    ...
    /// @brief Enable clock of a core in stop mode.
    ///
    void enableClockStop() const {
        auto reg = (__IO uint32_t *)(intptr_t(this->enableReg) + clockStopOffset);
        *reg  = *reg | this->flag;
    }

    /// @brief Disable clock of a core in stop mode.
    ///
    void disableClockStop() const {
        auto reg = (__IO uint32_t *)(intptr_t(this->enableReg) + clockStopOffset);
        *reg  = *reg & ~this->flag;
    }

    static constexpr int resetOffset = (offsetof(RCC_TypeDef, AHB1RSTR1) - offsetof(RCC_TypeDef, AHB1ENR1));
    static constexpr int clockSleepOffset = (offsetof(RCC_TypeDef, AHB1SLPENR1) - offsetof(RCC_TypeDef, AHB1ENR1));
    static constexpr int clockStopOffset = (offsetof(RCC_TypeDef, AHB1STPENR1) - offsetof(RCC_TypeDef, AHB1ENR1));
};
...
struct Info2 {
    ...
    ///
    void enableClockStop() const {
        auto reg = (__IO uint32_t *)(RCC_BASE + R + clockStopOffset);
        *reg = *reg | this->flag;
    }
